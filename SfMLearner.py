...
class SfMLearner(object):
    ...
    def get_learning_rate(self, batch, BASE_LEARNING_RATE, BATCH_SIZE):
    ...
    def build_train_graph(self):
        ...
        with tf.name_scope("depth_prediction"):
            ...
            #                                           is_training=True)
            pred_disp = dispRes_net(tgt_image, 
                                    is_training=True)
        ...
        with tf.name_scope("compute_loss"):
            ...
            for s in range(opt.num_scales):
                ...
                for i in range(opt.num_source):
                    ...
                    ########## Adding SSIM Loss ########################
                    ssim1 = tf.image.ssim(curr_proj_image, curr_tgt_image, max_val=1.0)
                    ssim_loss += (1-tf.reduce_mean(ssim1))/2
                    ####################################################
            ...
            total_loss = 0.15*pixel_loss + 0.5*smooth_loss + exp_loss + 0.85*ssim_loss

        with tf.name_scope("train_op"):
            # batch = tf.Variable(0)
            # bn_decay = self.get_bn_decay(batch, opt.batch_size)
            # learning_rate = self.get_learning_rate(self.global_step, opt.learning_rate, opt.batch_size)
    ...
    def collect_summaries(self):
        ...
        # tf.summary.scalar("incr_step", self.incr_global_step)
        for s in range(opt.num_scales):
